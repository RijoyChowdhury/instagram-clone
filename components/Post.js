...
function Post({ id, username, userImg, img, caption }) {
    ...
    useEffect(() => setHasLiked(likes.findIndex(like => (like.data().username === session?.user?.username)) !== -1), [likes, db]);
    ...
    return (
        <div className='bg-white my-7 border rounded-sm'>
            ...
            <div className='flex items-center p-5'>
                <img className='rounded-full h-12 w-12 object-cover border p-1 mr-3' src={userImg} alt="post img" />
            ...
            {session && <div className='flex justify-between px-4 pt-4'>
                <div className='flex space-x-4'>
                    ...
                    <div className='relative btn'>
                        ...
                        {comments.length > 0 && <div className='badge'>{comments.length}</div>}
                    </div>
            ...
            <p className='p-5 truncate'>
                {likes.length > 0 && (
                    <div className='flex'>
                    <HeartIconFilled className='h-5 mr-1' />
                ...
                <div className='ml-10 h-20 overflow-y-scroll scrollbar-thumb-black scrollbar-thin'>
                    {comments.map(comment => (
                        <div key={comment.id} className='flex items-center space-x-2 mb-3'>
                            ...
                            <p className='text-sm flex-1 truncate'><span className='font-bold mr-2'>{comment.data().username}</span>{comment.data().comment}</p>
